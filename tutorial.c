...
bool dbCheck(Session* dataSession) {
    ...
    AggregateFuture* response = query_get_aggregate(tx, testQuery, opts);
    ...
    // Retrieve the result
    int64_t result = aggregate_future_get_long(response);
    if (FAILED()) {
        aggregate_future_drop(response);
        transaction_close(tx);
        options_drop(opts);
        return false;
    }
...
bool dbSetup(DatabaseManager* dbManager, const char* dbName, bool dbReset) {
    ...
    if (databases_contains(dbManager, dbName)) {
        ...
        bool checkResult = dbCheck(session);
...
// Fetch all users and return the count of users
...
void queries(DatabaseManager* dbManager, const char* dbName) {
